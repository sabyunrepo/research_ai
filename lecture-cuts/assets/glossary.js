(() => {
  const terms = [
    ...
    ["tool", "AI가 호출해 실제 파일 읽기, 웹 확인 같은 일을 하는 기능입니다."],
    ["test", "기능이 의도대로 작동하는지 자동으로 확인하는 코드나 명령입니다."],
  ].sort((a, b) => b[0].length - a[0].length);
  const explanations = new Map(terms.map(([term, explanation]) => [term.toLowerCase(), explanation]));
  const pattern = new RegExp(`(?<![A-Za-z0-9])(${terms.map(([term]) => term.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|")})(?![A-Za-z0-9])`, "gi");
  ...
  function showPopover(target) {
    ...
    const margin = 12;
    const left = Math.min(Math.max(rect.left + rect.width / 2 - popoverRect.width / 2, margin), win.innerWidth - popoverRect.width - margin);
  ...
  function hidePopover(target) {
    target.ownerDocument.querySelector("#glossaryPopover")?.classList.remove("is-visible");
  }

  function makeNode(doc, label) {
    const node = doc.createElement("span");
    const explanation = explanations.get(label.toLowerCase()) || "";
    ...
    node.addEventListener("mouseleave", () => hidePopover(node));
    node.addEventListener("focus", () => showPopover(node));
    node.addEventListener("blur", () => hidePopover(node));
    return node;
  }
  ...
  function apply(root) {
    ...
    textNodes.forEach((textNode) => {
      ...
      while ((match = pattern.exec(value))) {
        ...
        fragment.append(makeNode(doc, match[0]));
  ...
  window.LECTURE_GLOSSARY = { apply, terms };
})();
