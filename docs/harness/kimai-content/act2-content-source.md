...
```text
Act 1 책상 정리(Context) 회수 -> 업무 지시(Prompt) 시작 -> 모호한 지시 위험 -> 말하지 않은 부분의 상상 -> 입력/결과 원리 -> 인수인계서 비유 -> 목표와 사용 상황 -> 자료와 조건 -> 출력 형식과 완료 기준 -> Prompt 구조 정리 -> 프롬프트 재정의 -> 실습 진입
```
...
### 2-1. 업무 지시(Prompt)는 김아이가 바로 일하게 만드는 작업지시서입니다
...
```text
업무 지시(Prompt)는 김아이가 바로 일하게 만드는 작업지시서입니다.
```
...
일할 준비가 되었으면 업무 지시(Prompt)로 김아이에게 일을 맡긴다고 말한다.
```
...
### 2-9. 업무 지시(Prompt)는 조건이 빠지지 않아야 흔들리지 않습니다

Type: consolidation
Estimated time: 50초
Function: Prompt 구조 정리

Screen headline:

```text
업무 지시(Prompt)는 조건이 빠지지 않아야 흔들리지 않습니다.
```
...
```text
Act 0과 Act 2 첫 장에서 이미 본 업무 지시(Prompt)를 다시 회수한다.
프롬프트는 AI에게 이번 일을 맡기는 업무 지시이자 조건을 고정하는 작업 명세라고 정리한다.
목표, 자료, 조건, 출력 형식, 완료 기준이 빠지지 않아야 흔들리지 않는다고 말한다.
```
