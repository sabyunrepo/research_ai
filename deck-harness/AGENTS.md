# Deck Harness Instructions

## Scope

- 이 지침은 `deck-harness/`와 그 하위 파일에 적용한다.
- 루트 `AGENTS.md`의 공통 context research와 verification orchestrator 규칙을 함께 따른다.

## Generated Deck Workflow
...
- 이미지 자산은 먼저 `asset-pack.json`에 정의한다. 각 자산은 `id`, 설명 가능한 teaching role, 생성 프롬프트, 스타일 제약, 설명 앵커, source path 또는 crop metadata를 가져야 한다.
- 슬라이드는 이미지 파일 경로를 직접 반복하지 않고 `visualAssetId`로 참조한다.
...
## Prompt And Projection Boundaries
